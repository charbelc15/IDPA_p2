...
def get_TF_Indexing_PCC(strings,Query):
    ...
    indexed_Docs = order_docs(keywords_docIDs_TF)
    ...
    #Indexing : Get Aug TF and Similarity only of files that are in indexed_Docs 
    ...
    for key in indexed_Docs:
        ...
        #Get AugTF
        print("AugTF of Indexed Doc ", file_name, "\n", (All_aug_vectors_TF[key]))
        print()

        #Get Similarity
        Sim = PCC( TF_Query_dict , All_aug_vectors_TF[key])
        print("Similarity via PCC with Doc ", file_name, " : ", Sim)
        print()
